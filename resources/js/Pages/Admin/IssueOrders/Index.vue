<template>
    <table-layout
        title="Issue Orders"
        note="Issue products are confirmation that you convert the products from planned orders.
        You can edit the quantity of the products and delete the products.
        Moving the product to trash "
    >
        <Head title="Issue Orders" />
        <tooltip
    ...
    methods: {
        ...
        showConfirmationDialog(type) {
            ...
            if (type === "cancel") {
                ...
                this.message.title = `Are you sure you want to issue all orders?`;
                this.message.content =
                    "All data in the issue orders will be converted and save in receive products of specific branch.";
                this.isShow = true;
                return;
            }
            // check if there is selected planned orders
                ...
                if (this.selected_planned_orders.length <= 0) {
                    this.error.title = "No selected issue orders";
                    this.error.message =
                        "Please select at least one issue order to issue";
                    return;
                }
                this.message.title = `Are you sure you want to issue ${this.selected_planned_orders.length} orders?`;
                this.message.content =
                    "All data in the issue orders will be converted and save in receive products of specific branch.";
                this.isShow = true;
            }
        ...
        onConvertAll() {
            this.$inertia.visit(route("issue-orders.proceed"), {
                method: "POST",
                data: {
